...
class Chromosome:
    ...
    '''
    Теперь напишем функцию для генерации случайной хромосомы. Функция принимает два 
    параметра:
    длина хромосомы, которую надо получить, 
    и набор генов, из которого нужно сделать хромосому.
    '''
...
def cross(chromo1, chromo2):
    ...
    Его суть в том, чтобы каким-то образом перемешать гены двух родителей и 
    получить новую хромосому-потомка.

    Здесь также есть куча методов, и мы опять возьмём что попроще. 
    Это будет одноточечный кроссинговер.

    Мы выбираем случайную позицию внутри хромосомы, и потомок получает гены 
...
def mutate(population, chromo_count, gene_count, gene_pool):
    ...
    for i in range(chromo_count):
        ch_pos = randint(0, pop_size - 1)
        chromo = population[ch_pos]
...
на печать с порядковым номером и рейтингом:
'''
