# Использовние нескольких фикстур
...
def get_file(tmp_path):
    ...
    print(f'\n>> create file {f_name}')  # принтим в учебных целях
    with open(f_name, 'w+', encoding='utf-8') as f:
        yield f
    print(f'\n>> close file {f_name}')  # принтим в учебных целях


@pytest.fixture
def set_num(get_file):
    print(f'\n>> fill file {get_file.name} цифрами')  # принтим в учебных целях
    for i in range(10):
...
def set_char(get_file):
    print(f'\n>> fill file {get_file.name} буквами')  # принтим в учебных целях
